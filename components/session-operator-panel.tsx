...
type LiveStats = { targetCount: number; connected: number; completed: number; remaining: number; status: string };
...
export function SessionOperatorPanel({ classes }: { classes: SessionClass[] }) {
  ...
  const [classStats, setClassStats] = useState<Record<string, LiveStats>>({});
  const [localActive, setLocalActive] = useState<Record<string, boolean>>({});
  const selected = classes.find((item) => item.id === selectedId) ?? classes[0];
  if (!selected) return null;
  const current = active[selected.id];
  // QR 카드와 등록 일정 테이블이 같은 소스(로컬 시작 + 라이브 통계)로 상태를 표시하도록 통일
  const isRowActive = (item: SessionClass) => Boolean(localActive[item.id]) || (classStats[item.id] ? classStats[item.id].status === "ACTIVE" : item.status === "ACTIVE");
  const isActive = Boolean(current) || isRowActive(selected);
  const filteredClasses = statusFilter === "ALL" ? classes : classes.filter((item) => isRowActive(item) === (statusFilter === "ACTIVE"));
  const liveStats = classStats[selected.id] ?? null;

  useEffect(() => {
    let cancelled = false;
    async function refresh() {
      try {
        const response = await fetch("/api/admin/sessions?all=1", { cache: "no-store" });
        if (!response.ok) return;
        const body = await response.json() as { stats: Record<string, LiveStats> };
        if (!cancelled) setClassStats(body.stats ?? {});
  ...
  async function start() {
    ...
    try {
      ...
      const qr = await QRCode.toDataURL(result.url, { width: 420, margin: 1, color: { dark: "#0d3168", light: "#ffffff" } });
      setActive((previous) => ({ ...previous, [selected.id]: { id: result.id!, url: result.url!, qr } }));
      setLocalActive((previous) => ({ ...previous, [selected.id]: true }));
